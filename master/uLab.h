...
public:
    ...
    void SetSpeedStage(AXIS axis,
                       uint16_t speed,
                       DEVICE_CODE code = LOW_STAGE_CODE); //unit of speed: 0.12 mm/s
...
private slots:
    void RefreshPort();
    void ParsePort();
    void GetPosLowX() { GetPos(AXIS_X); }
    void GetPosLowY() { GetPos(AXIS_Y); }
    void GetPosLowZ() { GetPos(AXIS_Z); }
    void GetPosHighX() { GetPos(AXIS_X, HIGH_STAGE_CODE); }
    void GetPosHighY() { GetPos(AXIS_Y, HIGH_STAGE_CODE); }
    void GetPosHighZ() { GetPos(AXIS_Z, HIGH_STAGE_CODE); }
    void GetPresAndFlow();
